...
const removals = new Map<Element, Map<string, OnRemovalFn>>()
        ...
        case 'attributes': {
          {
            ...
            const requiredPrefix = datasetPrefix + (prefix ? `${prefix}-` : '')
            if (!attributeName?.startsWith(requiredPrefix)) {
              break
            }

            const el = target as HTMLorSVGElement
            const datasetKey = camel(attributeName.slice(datasetPrefix.length))
...
function applyAttributePlugin(el: HTMLorSVGElement, camelCasedKey: string) {
  // Extract the raw key from the dataset
  const rawKey = camelCasedKey.slice(prefix.length)
  ...
  if (removalFn) {
    let elRemovals = removals.get(el)
    if (!elRemovals) {
      elRemovals = new Map()
      removals.set(el, elRemovals)
    }
